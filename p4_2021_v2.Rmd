...
load(file = "C:/Users/oriol/Documents/Uni/4t Curs/Primer semestre/TAB/Definitivas/P4/Project4/data/pleuralCancer/rse_gene_pleura.Rdata")
...
Yes, with the head() function used on 'annotation' we can discover that from each gene, there are its symbol, length and id available.
</div>
...
<div style="background-color:#F0F0F0">
##### &emsp;<i class="fas fa-comment-dots"></i> Answer:
We have the total number of individuals along with each read, described each by the stage of their cancer, and whether this stage is 'early' or 'late'. Furthermore, we have not filtered any individual, but there are 58037 genes each with its symbol, id and bp length.
</div>
...
- **Do you observe any improvement when comparing the Raw data against the RPKM normalization?** No, the line diverges even more from y = 0.
- **And when comparing the Raw data and the TMM method?** Although they are quite similar, TMM is slightly better because we see the line closer to y= 0 than it is with the Raw Data.
- **Which is the best normalization method in this case?** TMM.
...
<div style="background-color:#F0F0F0">
##### &emsp;<i class="fas fa-comment-dots"></i> Answer:
From the original 58037 genes, 30 are differentially regulated: 1 underexpressed and 29 overexpressed.
</div>
...
<div style="background-color:#F0F0F0">
##### &emsp;<i class="fas fa-comment-dots"></i> Answer:
There are 34 genes, because each gene is a row and the dim() function fives us the rows and columns in the object.
</div>
