...
# yes, we are getting warning messages when we use alpha and size aesthetic attributes, because they are not recommended when using discrete variables
...
#The groups are stacked and the count corresponds to its frequency.
```
:::
...
#First we check the class of the variable
class(mtcars$gear) ##"numeric" -> we need to transform

#We make gear factor
mtcars$gear_f <- factor(mtcars$gear, levels = c(3,4,5))

#Plot
ggplot(data=mtcars, mapping=aes(x=cyl_f, fill=gear_f)) + geom_bar()
...
#The chart rotates, the best configuration is the vertical one, because the visualisation of data is easier 
...
#Our idea (set a different color for each group)
...
#I would use the scale_fill_hue() to ensure that colourblind people can distinguis the colours, because the colours are more different and the categories can be distinguished easier

# # scale_fill_hue() -> different colours for cut category
...
# # scale_fill_brewer() -> colour degradation (different tonalities of the same colour)
...
# Adding subtitle
 # p + scale_fill_manual(values = c("sienna1", "orange", "lightseagreen", "orangered", "red4")) +
 #   labs(title = "Diamond weight variation", x = "Weight of the diamond", fill = "Quality", subtitle = "This is a subtitle")
 
 # #Adding a caption
 # p + scale_fill_manual(values = c("sienna1", "orange", "lightseagreen", "orangered", "red4")) +
 #   labs(title = "Diamond weight variation", x = "Weight of the diamond", fill = "Quality", caption = "This is a caption")


# #Adding a tag
#   p + scale_fill_manual(values = c("sienna1", "orange", "lightseagreen", "orangered", "red4")) +
#    labs(title = "Diamond weight variation", x = "Weight of the diamond", fill = "Quality", tag = "This is a tag")
  
#All of them
   p + scale_fill_manual(values = c("sienna1", "orange", "lightseagreen", "orangered", "red4")) +
   labs(title = "Diamond weight variation", x = "Weight of the diamond", fill = "Quality", subtitle = "This is a subtitle", caption= "This is a caption", tag = "This is a tag")
...
```{r answer5.3}
# We think it's the themelight since the data visualization is easier.
```
:::
...
#Raster format
ggsave(filename = "plot_raster.png", plot= p, width = 6, height = 4, res=150) # In inches by default 
...
#Plot
#ggplot(data=economics, mapping = aes(y=unemploy,x=date))+geom_line()


economics_plot <- ggplot(data=economics, mapping = aes(y=unemploy,x=date))+geom_line(color="firebrick")
economics_plot + scale_x_continuous(name = "Year") + scale_y_continuous(name = "Unemployment")+ggtitle("Evolution of unemployment") + theme_light()

#Save image
ggsave(filename = "evolution_unemployment_plot.png", plot = economics_plot, width = 6, height = 4)
```
:::
...
HairEyeColor_df <- as.data.frame(HairEyeColor) #Transform dataset table to dataframe
# head(HairEyeColor_df)
# class(HairEyeColor_df) #df

# First we check the class of Hair
class(HairEyeColor_df$Eye) #factor

 ggplot(data = HairEyeColor_df, mapping = aes(x = Eye, y=Freq, fill=Sex)) +
   geom_col() +
   facet_grid(.~ Hair) +
   labs(title="Hair and eye color", x="Eye color", y="Frequency",caption="Source: HairEyeColor") +
   theme_light() +
