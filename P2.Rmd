...
##IT WORKS, BUT IT RECOMMENDS US NOT TO REPRESENT IT WITH 'ALPHA' AND 'SIZE', PROBABLY BECAUSE THE REPRESENTATION IS HARD TO SEE##.
```
:::
...
##THIS REARRANGES THE GRAPH SO THAT ONE CAN SEE THE PROPORTION (OUT OF 1) IN WHICH DATA IS DISTRIBUTED WITHIN GROUPS.##
```
:::
...
ggplot(data = mtcars, mapping = aes(x = cyl_f, fill = gear_f)) +
  geom_bar(position = "fill")
#WE DO NEED TO TRANSFORM IT INTO A FACTOR VARIABLE, AS WE ARE USING GEO_BAR, WHICH IS FOR DISCRETE VARIABLES, WHICH SHOULD BE FACTORS, AND NOT CONTINUOUS ONES (FACTOR)). Otherwise, the graph shows no aggrupation.##
```
:::
...
ggplot(data = diamonds, mapping = aes(x = carat, fill = cut)) +
  geom_histogram(position = "dodge", binwidth = 0.3) + 
  facet_grid(. ~ cut)
#IT PROVIDES A DIFFERENT (VERTICAL) VIEW.#
```
:::
...
::: {style="background-color:#F0F0F0"}
##### <i class="fas fa-comment-dots"></i> Answer:
::: I would say the vertical (previous) one because you have each 'cut' separated and a vertical views allows you to directly compare the different types, as they are next to each other.
...
#Using 'colour' inside 'mapping', we will fill the borders with a colour.#
```
:::
...
#Because the colors are more vibrant and different, the viridis_d palette is better for colorblind people#
```
:::
...
p + scale_fill_manual(values = c("sienna1", "orange", "lightseagreen", "orangered", "red4")) +
  labs(title = "Diamond weight variation", x = "Weight of the diamond", fill = "Quality", subtitle = "subtitle", caption = "caption", tag = "tag")

#They are basically adding a subtitle (down the title to further describe), a tag (something to identify the graph faster without the title) and a caption (which may further describe what the graph shows, its results or specify some details in it.)#
...
#Grey is the most appealing to my vision, and the one I think best shows the results, optimizing the data-ink ratio.#
```
:::
...
ggsave(filename = "plot6v.svg", plot = p, width = 6, height = 4, limitsize = FALSE)
...
head(economics$unemploy)
head(economics$date)
class(economics$unemploy)
class(economics$date)
v <- ggplot(data = economics, mapping = aes(x = date, y = unemploy)) +
  geom_line() +
  labs(title = "Evolution of unemployment", x = "Year", y = "Unemployment") +
  theme_bw() 
ggsave(filename = "wrapup.png", plot = v, width = 6, height = 4)
...
read.csv("HairEyeColor.csv", header = TRUE, sep = ",")
??HairEyeColor
read.table('HairEyeColor.txt')
head(HairEyeColor)
head(HairEyeColor_data_frame)
class(HairEyeColor_data_frame)
HairEyeColor_data_frame <- as.data.frame(table(HairEyeColor))
table(HairEyeColor)
```
